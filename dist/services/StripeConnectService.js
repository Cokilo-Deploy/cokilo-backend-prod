...
class StripeConnectService {
    ...
    static async createConnectedAccount(userId) {
        try {
            ...
            await User_1.User.update({ stripeConnectedAccountId: account.id }, { where: { id: userId } });
            return account.id;
        }
        catch (error) {
    ...
    static async createConnectedAccountWithUserData(userId, userIp) {
        try {
            const user = await User_1.User.findByPk(userId);
            if (!user) {
                throw new Error('Utilisateur non trouvé');
     ...
     */
    static async transferToTraveler(userId, amount, targetCurrency, transactionId) {
        try {
     ...
     */
    static async addExternalAccount(userId, bankDetails) {
        try {
            ...
            const externalAccount = await stripe.accounts.createExternalAccount(user.stripeConnectedAccountId, {
                external_account: {
                    object: 'bank_account',
                    country: bankDetails.country || 'FR',
                    currency: 'eur',
     ...
     */
    static async createPayout(userId, amount) {
        try {
            const user = await User_1.User.findByPk(userId);
            if (!user?.stripeConnectedAccountId) {
                throw new Error('Compte Connect non trouvé');
        ...
        catch (error) { // Changé ici : ajouté ": any"
     ...
     */
    static async getPayoutHistory(userId) {
        try {
            const user = await User_1.User.findByPk(userId);
            if (!user?.stripeConnectedAccountId) {
                throw new Error('Compte Connect non trouvé');
     ...
     */
    static mapPayoutStatus(stripeStatus) {
        switch (stripeStatus) {
            case 'paid':
                return 'completed';
            case 'pending':
