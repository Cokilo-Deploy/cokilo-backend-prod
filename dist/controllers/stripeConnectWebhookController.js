...
exports.handleStripeConnectWebhook = void 0;
...
const handleStripeConnectWebhook = async (req, res) => {
...
exports.handleStripeConnectWebhook = handleStripeConnectWebhook;
...
async function handleTransferCreated(transfer) {
    ...
    try {
        ...
        await Transaction_1.Transaction.update({
...
async function handleTransferReversed(transfer) {
    ...
    try {
        ...
        if (transaction && transaction.traveler) {
            ...
            await walletService_1.WalletService.creditWallet(transaction.traveler.id, // userId: number
...
async function handleAccountUpdated(account) {
    ...
    try {
        ...
        if (user && account.details_submitted && account.charges_enabled) {
            ...
            await User_1.User.update({ paymentMethod: 'stripe_connect' }, { where: { id: user.id } });
        }
    }
    catch (error) {
