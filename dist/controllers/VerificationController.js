...
const StripeConnectService_1 = require("../services/StripeConnectService"); // AJOUT
...
class VerificationController {
    ...
    static async startVerification(req, res) {
        try {
            ...
            // Méthode 2: Détecter depuis les headers Cloudflare
                ...
                userCountry = req.headers['cf-ipcountry'].toUpperCase();
            }
            // Méthode 3: Depuis les headers de géolocalisation personnalisés
            else if (req.headers['x-country']) {
                userCountry = req.headers['x-country'].toUpperCase();
            }
    ...
    static async updateConnectedAccountWithIdentityData(userId, verificationSessionId) {
        try {
            ...
            if (verificationSession.status === 'verified' && verificationSession.verified_outputs) {
                ...
                if (verificationSession.last_verification_report) {
                    const report = await stripe.identity.verificationReports.retrieve(verificationSession.last_verification_report);
                    ...
                    if (report.document) {
                        ...
                        if (report.document.front) {
                            updateData.individual.verification.document.front = report.document.front;
                        }
                        if (report.document.back) {
                            updateData.individual.verification.document.back = report.document.back;
                        }
                    }
                }
                        ...
                        switch (session.status) {
                            case 'verified':
                                ...
                                if (isEuropeanUser && !user.stripeConnectedAccountId) {
                                    try {
                                        const connectAccount = await StripeConnectService_1.StripeConnectService.createConnectedAccountWithUserData(user.id, '127.0.0.1' // IP par défaut pour webhook
                                        );
                                        await StripeConnectService_1.StripeConnectService.updateConnectedAccountWithIdentityData(user.id, session.id);
                                        await user.update({
                                            verificationStatus: newStatus,
    ...
    static async submitStripeData(req, res) {
        try {
            const user = req.user;
            const { dateOfBirth, addressLine1, addressLine2, addressCity, addressPostalCode, acceptStripeTerms, phone, state } = req.body;
            ...
            const normalizedPhone = (0, phoneUtils_1.normalizePhoneNumber)(phone);
            ...
            const existingPhone = await User_1.User.findOne({
                where: {
                    ...
                    id: { [sequelize_1.Op.ne]: user.id } // Exclure l'utilisateur actuel
                }
            });
            if (existingPhone) {
            ...
            if (validPrefixes && !validPrefixes.some(prefix => phone.startsWith(prefix))) {
                ...
                return res.status(400).json({
                    success: false,
                    errorCode: errorCodes_1.ErrorCode.INVALID_PHONE_PREFIX,
                    data: {
                        country: user.country,
            ...
            if (postalCodeRules) {
                ...
                if (!isLengthValid || !isPatternValid) {
                    ...
                    // Pas de sauvegarde - retour direct
                    const examplePostalCode = user.country === 'ES' ? '28001' :
                    ...
                    return res.status(400).json({
                        success: false,
                        errorCode: errorCodes_1.ErrorCode.INVALID_POSTAL_CODE_FORMAT,
                        data: {
                            country: user.country,
            ...
            if (isEuropeanUser) {
                ...
                try {
                    const connectAccountId = await StripeConnectService_1.StripeConnectService.createConnectedAccountWithUserData(user.id, req.ip || '127.0.0.1');
                    await user.update({
