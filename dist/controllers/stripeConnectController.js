...
class StripeConnectController {
    ...
    static async initiateOnboarding(req, res) {
        try {
            ...
            if (!accountId) {
                accountId = await StripeConnectService_1.StripeConnectService.createConnectedAccount(userId);
            }
            // Créer le lien d'onboarding
            const onboardingUrl = await StripeConnectService_1.StripeConnectService.createOnboardingLink(userId);
            res.json({
                success: true,
                onboardingUrl,
                accountId,
    ...
    static async getAccountStatus(req, res) {
        try {
            ...
            const status = await StripeConnectService_1.StripeConnectService.getAccountStatus(userId);
            const user = await User_1.User.findByPk(userId);
            res.json({
                success: true,
    ...
    static async updateCountry(req, res) {
        try {
            ...
            await User_1.User.update({ country: country.toUpperCase() }, { where: { id: userId } });
    ...
    static async getPaymentInfo(req, res) {
        try {
            ...
            const accountStatus = user.stripeConnectedAccountId
                ? await StripeConnectService_1.StripeConnectService.getAccountStatus(userId)
                : null;
            res.json({
                success: true,
                paymentInfo: {
