...
const ExtendedRegistrationService_1 = require("../services/ExtendedRegistrationService");
...
class AuthController {
    ...
    static async detectCountryFromIP(req) {
        try {
            ...
            const response = await axios_1.default.get(`http://ip-api.com/json/${realIP}?fields=countryCode`, {
                timeout: 3000
            });
    ...
    static async createAndValidateStripeConnect(data, userIp) {
        const errors = [];
        try {
    ...
    static async register(req, res) {
        try {
            ...
            const result = await ExtendedRegistrationService_1.ExtendedRegistrationService.registerWithStripeConnect(req.body, Array.isArray(userIp) ? userIp[0] : userIp.toString());
            const userAccess = (0, userAccess_1.getUserAccessInfo)(result.user);
            res.status(201).json({
                success: true,
                data: {
                    token: result.token,
    ...
    static async registerSimple(req, res) {
        try {
            ...
            const verificationCode = EmailVerificationService_1.EmailVerificationService.generateVerificationCode();
            const codeExpiration = EmailVerificationService_1.EmailVerificationService.getCodeExpiration();
            ...
            const emailSent = await EmailVerificationService_1.EmailVerificationService.sendVerificationCode(email, firstName, verificationCode);
    ...
    static async verifyEmail(req, res) {
        try {
            ...
            const isValid = EmailVerificationService_1.EmailVerificationService.isCodeValid(verificationCode, user.verificationCode || '', user.verificationCodeExpires || new Date());
            if (!isValid) {
                return res.status(400).json((0, errorCodes_1.errorResponse)(errorCodes_1.ErrorCode.INVALID_VERIFICATION_CODE));
            }
    ...
    static async resendVerification(req, res) {
        try {
            ...
            await EmailVerificationService_1.EmailVerificationService.sendVerificationCode(user.email, user.firstName, verificationCode);
            res.json({
                success: true,
    ...
    static async login(req, res) {
        try {
            ...
            if (!user) {
                return res.status(401).json((0, errorCodes_1.errorResponse)(errorCodes_1.ErrorCode.INVALID_CREDENTIALS));
            }
            ...
            if (!user.emailVerifiedAt) {
                ...
                return res.status(403).json({
                    success: false,
                    errorCode: errorCodes_1.ErrorCode.EMAIL_NOT_VERIFIED,
                    requiresVerification: true,
                    userId: user.id,
                    email: user.email
                });
    ...
    static async changePassword(req, res) {
        try {
            ...
            if (!isCurrentPasswordValid) {
                return (0, responseHelpers_1.sendLocalizedResponse)(res, 'msg.incorrect_current_password', null, 400, user);
            }
    ...
    static async checkAccountDeletion(req, res) {
        try {
            ...
            const activeTransactions = await Transaction_1.Transaction.findAll({
                where: {
                    [sequelize_1.Op.or]: [
                        { senderId: userId },
                        { travelerId: userId }
                    ...
                    status: {
                        [sequelize_1.Op.in]: ['payment_pending', 'payment_escrowed', 'package_picked_up']
                    }
                }
            });
            ...
            let walletBalance = 0;
            try {
                const balanceDetails = await walletService_1.WalletService.getDetailedWalletBalance(userId);
                walletBalance = balanceDetails.totalBalance;
            }
            catch (walletError) {
                console.log('Erreur vérification wallet:', walletError);
                walletBalance = await walletService_1.WalletService.getWalletBalance(userId);
            }
    ...
    static async deleteAccount(req, res) {
        try {
            ...
            await ChatMessage_1.ChatMessage.destroy({
                where: { senderId: userId }
            });
            // 2. Conversations de chat
            console.log('Suppression conversations...');
            await ChatConversation_1.ChatConversation.destroy({
                where: {
                    [sequelize_1.Op.or]: [
                        { user1Id: userId },
                        { user2Id: userId }
